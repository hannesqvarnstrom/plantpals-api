...
import IdentityModel, { type TCreateIdentityPayload } from "../models/identity";
...
export class AuthenticationService {
	...
	public async attemptPasswordLogin({
		email,
		password,
	}: IPasswordLoginBody): Promise<TUser> {
		const user = await this.userModel.getByEmail(email);
		const genericLoginErrorMessage = new AppError(
