...
import { families, genera, species } from "../../db/schema";
...
export const SCRAPING_URLS = {
	family_wikipedia:
		"https://en.wikipedia.org/wiki/List_of_plant_family_names_with_etymologies",
} as const;

const getDataFromID = (id: string | number) =>
	`https://api.gbif.org/v1/species/${id}`;
const getAllChildrenURL = (id: string | number) =>
	`https://api.gbif.org/v1/species/${id}/childrenAll`;

export async function scrapeFamilyDataAll() {
	const fileName = `${__dirname}/families.json`;
	const familyNames: string[] = await fs
		.readFile(fileName)
		.then((data) => JSON.parse(data.toString()));

	const searchUrl =
		"https://techdocs.gbif.org/en/openapi/v1/species#/Searching%20names/searchNames";
	...
	for (const familyName of familyNames) {
		...
		const { data: familyRequestData } = await makeRequest(
			`https://api.gbif.org/v1/species/search?datasetKey=d7dddbf4-2cf0-4f39-9b2a-bb099caae36c&q=${familyName}`,
		);
		console.log("request complete");
		const likelyfamily = familyRequestData.results.find(
			(item: TaxonomicRecord) =>
				item.rank === "FAMILY" && item.taxonomicStatus === "ACCEPTED",
		);
		if (!likelyfamily) {
...
async function selectOrCreateFamily(familyData: TaxonomicRecord) {
	const [family, ..._] = await dbManager.db
		.select()
		.from(families)
		.where(eq(families.gbifKey, familyData.key.toString()));
	if (family) {
		return family;
	}
	const [createdFamily, ..._1] = await dbManager.db
		.insert(families)
		.values({
			name: familyData.canonicalName,
			gbifKey: familyData.key.toString(),
			vernacularNames: familyData.vernacularNames,
		})
		.returning();
	if (createdFamily) {
	...
	throw new Error(
		`could not create family with name ${familyData.canonicalName}`,
	);
}

async function selectOrCreateGenus(
	genusData: TaxonomicRecord,
	familyId: number,
) {
	const [genus, ..._] = await dbManager.db
		.select()
		.from(genera)
		.where(eq(genera.gbifKey, genusData.key.toString()));
	if (genus) {
		return genus;
	}
	try {
		const [createdGenus, ..._] = await dbManager.db
			.insert(genera)
			.values({
				name: genusData.canonicalName,
				gbifKey: genusData.key.toString(),
				vernacularNames: genusData.vernacularNames,
				familyId,
				gbifFamilyKey: genusData.familyKey.toString(),
			})
			.returning();
		if (createdGenus) {
			return createdGenus;
		...
		throw new Error(
			`could not create genus with name ${genusData.canonicalName}`,
		);
	} catch (e) {
...
export async function scrapeFamilySpecific(familyName: string) {
	 ...
	 * 1. Create family
	 */
	const familiesData: TaxonomicRecord[] = await fs
		.readFile(`${__dirname}/extracted-family-data.json`)
		.then((data) => JSON.parse(data.toString()));
	const familySpecificData = familiesData.find(
		(family) => family.canonicalName.toLowerCase() === familyName.toLowerCase(),
	);
	...
	const { data: shallowGenera } = (await makeRequest(
		getAllChildrenURL(family.gbifKey),
	)) as { data: TaxonomicRecord[] };
	...
	for (const genus of shallowGenera) {
		 ...
		 */
		const { data: genusData } = (await makeRequest(
			getDataFromID(genus.key),
		)) as { data: TaxonomicRecord };
		if (
			genusData.taxonomicStatus === "ACCEPTED" &&
			genusData.rank === "GENUS"
		) {
			...
			const { data: genusChildData } = await makeRequest(
				getAllChildrenURL(genusData.key),
			);
			...
			for (const speciesItem of genusChildData) {
				 ...
				 */
				const nestedSpeciesData = createSpeciesAndChildren(
					speciesItem.key,
					genusInDB.id,
					family.id,
				);
...
function validateSpecies(speciesData: TaxonomicRecord) {
	...
	return (
		requiredCols.every(
			(col) => !!speciesData[col as keyof typeof speciesData],
		) && acceptedRanks.includes(speciesData.rank)
	);
}

async function createSpeciesAndChildren(
	speciesKey: number,
	genusId: number,
	familyId: number,
	relatedSpeciesId?: number,
) {
	const [speciesCreated, ..._] = await dbManager.db
		.select()
		.from(species)
		.where(eq(species.gbifKey, String(speciesKey)))
		.execute();
	if (speciesCreated) {
	...
	if (speciesData.taxonomicStatus === "ACCEPTED") {
		...
		if (!validationResult) {
			console.error("could not validate species", {
				key: speciesData.key,
				name: speciesData.canonicalName,
			});
			return;
		}
		let createdSpecies: TSpecies;
		try {
			const [creationResult, ..._] = await dbManager.db
				.insert(species)
				.values({
					name: speciesData.canonicalName,
					genusId: genusId,
		...
		console.log(
			`created species: ${createdSpecies?.name}, key: ${createdSpecies?.gbifKey}`,
		);
		if (speciesData.numDescendants) {
			const { data: subSpeciesData } = await makeRequest(
				getAllChildrenURL(String(speciesKey)),
			);
			const children: unknown[] = [];
			for (const child of subSpeciesData) {
				const result = await createSpeciesAndChildren(
					child.key,
					genusId,
					familyId,
					createdSpecies?.id,
	...
	console.warn(
		`skipping synonym with name : ${speciesData.canonicalName}, key: ${speciesData.key}`,
	);
	return;
}
...
export async function scrapeFamilyNames() {
	...
	for (const { attribs: properties } of familiesTable) {
		try {
			...
			console.error("could not get title from element", {
				error: e,
				properties,
			});
		}
